...
const { TOKEN_PARAMS,
	...
	EXTERNAL_TOKEN_PARAMS } = require("../params");
...
describe("StakingRewards contract", function () {
	...
	before(async () => {
      ...
      STAKING_TOKEN_PARAMS.ADDRESS
		...
		const signer2 = await ethers.provider.getSigner(STAKING_TOKEN_PARAMS.MAIN_HOLDER);
		...
		const signer3 = await ethers.provider.getSigner(EXTERNAL_TOKEN_PARAMS.MAIN_HOLDER);
		externalRewardsTokenOwner = await externalRewardsToken.connect(signer3);

		await stakingRewards.connect(owner).setRewardsDistribution(mockRewardsDistributionAddress.address);
	});
	...
	describe('Function permissions', () => {
		...
		it('only rewardsDistribution address can call notifyRewardAmount', async () => {
			await expect(stakingRewards.connect(mockRewardsDistributionAddress).notifyRewardAmount(rewardValue)).to.emit(
				stakingRewards,
				"RewardAdded"
			);
			assert.revert(
		...
		it('only owner address can call setRewardsDuration', async () => {
			...
			await expect(stakingRewards.connect(owner).setRewardsDuration(70)).to.emit(
				stakingRewards,
	...
	describe('Pausable', async () => {
		...
		it('should revert calling stake() when paused', async () => {
			...
			await stakingToken.connect(deployerAccount).approve(stakingRewards.address, totalToStake);
	...
	describe('External Rewards Recovery', () => {
		...
		it('only owner can call recoverERC20', async () => {
			await expect(await stakingRewards.connect(owner).recoverERC20(externalRewardsToken.address, amount)).to.emit(
				stakingRewards,
				"Recovered"
			);
			assert.revert(
				stakingRewards.connect(deployerAccount).recoverERC20(externalRewardsToken.address, amount),
				"Only the contract owner may perform this action"
			);
		});
		...
		it('should retrieve external token from StakingRewards and reduce contracts balance', async () => {
			...
			assert.bnEqual(await externalRewardsToken.balanceOf(stakingRewards.address), "0");
		});
		...
		it('should emit Recovered event', async () => {
			await expect(stakingRewards.connect(owner).recoverERC20(externalRewardsToken.address, amount)).to.emit(
				stakingRewards,
				"Recovered"
			);
		});
	...
	describe('lastTimeRewardApplicable()', () => {
		...
		describe('when updated', () => {
			it('should equal current timestamp', async () => {
				await stakingRewards.connect(mockRewardsDistributionAddress).notifyRewardAmount(toUnit('1'));
	...
	describe('rewardPerToken()', () => {
		...
		it('should be > 0', async () => {
			...
			await rewardsTokenOwner.transfer(stakingRewards.address, rewardValue);
			await stakingRewards.connect(mockRewardsDistributionAddress).notifyRewardAmount(rewardValue);
	...
	describe('stake()', () => {
		it('staking increases staking balance', async () => {
			...
			const initialStakeBal = await stakingRewards.balanceOf(deployerAccount.address);
			const initialLpBal = await stakingToken.balanceOf(deployerAccount.address);

			await stakingRewards.connect(deployerAccount).stake(totalToStake);

			const postStakeBal = await stakingRewards.balanceOf(deployerAccount.address);
		...
		it('cannot stake 0', async () => {
			await assert.revert(stakingRewards.connect(owner).stake('0'), 'Cannot stake 0');
		});
	});
	...
	describe('earned()', () => {
		...
		it('rewardRate should increase if new rewards come before DURATION ends', async () => {
			...
			await rewardsTokenOwner.transfer(stakingRewards.address, totalToDistribute);
			await stakingRewards.connect(mockRewardsDistributionAddress).notifyRewardAmount(totalToDistribute);
	...
	describe('getReward()', () => {
		it('should increase rewards token balance', async () => {
			...
			const initialRewardBal = await rewardsToken.balanceOf(deployerAccount.address);
			const initialEarnedBal = await stakingRewards.earned(deployerAccount.address);
			await stakingRewards.connect(deployerAccount).getReward();
			const postRewardBal = await rewardsToken.balanceOf(deployerAccount.address);
			const postEarnedBal = await stakingRewards.earned(deployerAccount.address);

			assert.isAbove(parseInt(initialEarnedBal.toString()), parseInt(postEarnedBal.toString()));
			assert.isAbove(parseInt(postRewardBal.toString()), parseInt(initialRewardBal.toString()));
		});
	});
	...
	describe('setRewardsDuration()', () => {
		...
		it('should update when setting setRewardsDuration after the period has finished', async () => {
			...
			await expect(stakingRewards.connect(owner).setRewardsDuration(seventyDays)).to.emit(
				stakingRewards,
				"RewardsDurationUpdated"
			);
	...
	describe('withdraw()', () => {
		it('cannot withdraw if nothing staked', async () => {
			await expect(stakingRewards.connect(owner).withdraw(toUnit('100'))).to.be.reverted;
		});
		...
		it('should increases lp token balance and decreases staking balance', async () => {
			...
			const initialStakingTokenBal = await stakingToken.balanceOf(deployerAccount.address);
			const initialStakeBal = await stakingRewards.balanceOf(deployerAccount.address);

			await stakingRewards.connect(deployerAccount).withdraw(totalToStake);

			const postStakingTokenBal = await stakingToken.balanceOf(deployerAccount.address);
			const postStakeBal = await stakingRewards.balanceOf(deployerAccount.address);

			assert.equal(BigNumber.from(postStakeBal).add(BigNumber.from(totalToStake)).toString(), initialStakeBal.toString());
			assert.equal(BigNumber.from(initialStakingTokenBal).add(BigNumber.from(totalToStake)).toString(), postStakingTokenBal.toString());
		});

		it('cannot withdraw 0', async () => {
			await assert.revert(stakingRewards.connect(owner).withdraw('0'), 'Cannot withdraw 0');
		});
	});
	...
	describe('exit()', () => {
		it('should retrieve all earned and increase rewards bal', async () => {
			...
			await rewardsTokenOwner.transfer(stakingRewards.address, totalToDistribute);
			await stakingRewards.connect(mockRewardsDistributionAddress).notifyRewardAmount(toUnit(5000.0));
	...
	describe('notifyRewardAmount()', () => {
		...
		before(async () => {
			...
			await localStakingRewards.connect(owner).setRewardsDistribution(mockRewardsDistributionAddress.address);
		});

		it('Reverts if the provided reward is greater than the balance.', async () => {
			const rewardValue = toUnit(1000);
			await rewardsTokenOwner.transfer(localStakingRewards.address, rewardValue);
			await assert.revert(
				localStakingRewards.connect(mockRewardsDistributionAddress).notifyRewardAmount(rewardValue.add(toUnit(0.1))),
		...
		it('Reverts if the provided reward is greater than the balance, plus rolled-over balance.', async () => {
			const rewardValue = toUnit(1000);
			await rewardsTokenOwner.transfer(localStakingRewards.address, rewardValue);
			localStakingRewards.connect(mockRewardsDistributionAddress).notifyRewardAmount(rewardValue);
			await rewardsTokenOwner.transfer(localStakingRewards.address, rewardValue);
