...
describe("Fee Custody", () => {
  ...
  describe("checking public variables", () => {
    ...
    it("returns emergency return address", async () => {
      expect(await feeCustody.owner()).eq(
        sa.fundManager.address
      );
    });

    it("returns keeper", async () => {
      expect(await feeCustody.keeper()).eq(
        sa.fundManager2.address
      );
    });
  });
  ...
  describe("Setters", () => {
    ...
    it("it sets setAsset", async () => {
      ...
      let pathEncodePacked = ethers.utils.solidityPack(["address", "address", "uint256"], [USDC_ADDRESS, WETH_ADDRESS, POOL_LARGE_FEE])
    ...
    it("it sets setAsset with intermediaryPath", async () => {
      ...
      let pathEncodePacked = ethers.utils.solidityPack(["address", "uint256", "address", "uint256", "address"], [USDC_ADDRESS, POOL_LARGE_FEE, WBTC_ADDRESS, POOL_LARGE_FEE, WETH_ADDRESS])
  ...
  describe("Protocol Revenue Distribution", () => {
    ...
    it("it gets correct amount of asset claimable by RBN lockers", async () => {
      ...
      expect(await feeCustody.claimableByRBNLockersOfAsset(WETH_ADDRESS)).eq(BigNumber.from(totalWETHDeposited).mul(await feeCustody.pctAllocationForRBNLockers()).div(await feeCustody.TOTAL_PCT()));
    });
    ...
    it("it gets correct amount of asset claimable by protocol revenue recipient", async () => {
      ...
      expect(await feeCustody.claimableByProtocolOfAsset(WETH_ADDRESS)).eq(BigNumber.from(totalWETHDeposited).mul(totalPCT.sub(await feeCustody.pctAllocationForRBNLockers())).div(totalPCT));
    });
    ...
    it("it gets correct amount of asset claimable by RBN lockers", async () => {
      ...
      for (let asset in ASSET_TO_CHAINLINK) {
          const oracle = await ethers.getContractAt("IChainlink", ASSET_TO_CHAINLINK[asset]);
          const token = await ethers.getContractAt("IERC20", asset);
          ...
          totalInUSD = totalInUSD.add(BigNumber.from(bal)
    ...
    it("it gets correct amount of asset claimable by protocol revenue recipient", async () => {
      let totalInUSD = BigNumber.from(0);
      let totalPCT = await feeCustody.TOTAL_PCT();
      let allocPCT = totalPCT.sub(await feeCustody.pctAllocationForRBNLockers());
    ...
    it("it swaps and distributes protocol revenue to protocol revenue recipient & fee distributor", async () => {
        ...
        for (let asset in ASSET_TO_CHAINLINK) {
          prevAdminAlloc[asset] = await feeCustody.claimableByProtocolOfAsset(asset);
        }
        ...
        const oracle = await ethers.getContractAt("IChainlink", ASSET_TO_CHAINLINK[WETH_ADDRESS]);

        // make sure eth sent to fee distributor (the amount they deserve +/- something)
        let feeDistributorBalanceInUSD = (await provider.getBalance(feeDistributor.address)).mul(await oracle.lastestAnswer()).div(BigNumber.from(10).mul(BigNumber.from(10).pow(8)))
        expect(feeDistributorBalanceInUSD).to.be.above(totalClaimableByRBNLockersInUSD.mul(100 - slippage).div(100))
        expect(feeDistributorBalanceInUSD).to.be.below(totalClaimableByRBNLockersInUSD)
    });
