  ...
  describe('#claim', () => {
    ...
    describe('two account tree', () => {
      ...
      it('must have enough to transfer', async () => {
        ...
        await expect(localDistributor.claim(0, wallet0.address, 100, proof0)).to.be.revertedWith(
      ...
      it('cannot claim more than once: 0 and then 1', async () => {
        ...
        await expect(
          localDistributor.claim(0, wallet0.address, 100, tree.getProof(0, wallet0.address, BigNumber.from(100)))
      ...
      it('cannot claim more than once: 1 and then 0', async () => {
        ...
        await expect(
          localDistributor.claim(1, wallet1.address, 101, tree.getProof(1, wallet1.address, BigNumber.from(101)))
        ).to.be.revertedWith('MerkleDistributor: Drop already claimed.')
      })
    ...
    describe('realistic size tree', () => {
      ...
      it('gas average random distribution', async () => {
        ...
        for (let i = 0; i < NUM_LEAVES; i += NUM_LEAVES / NUM_SAMPLES) {
          ...
          const tx = await localDistributor.claim(i, wallet0.address, 100, proof)
          const receipt = await tx.wait()
      ...
      it('no double claims in random distribution', async () => {
        for (let i = 0; i < 25; i += Math.floor(Math.random() * (NUM_LEAVES / NUM_SAMPLES))) {
          ...
          await expect(localDistributor.claim(i, wallet0.address, 100, proof)).to.be.revertedWith(
            'MerkleDistributor: Drop already claimed.'
          )
        }
      })
  ...
  describe('parseBalanceMap', () => {
    ...
    it('all claims work exactly once', async () => {
      for (let account in claims) {
        ...
        await expect(localDistributor.claim(claim.index, account, claim.amount, claim.proof))
          .to.emit(localDistributor, 'Claimed')
          .withArgs(claim.index, account, claim.amount)
        await expect(localDistributor.claim(claim.index, account, claim.amount, claim.proof)).to.be.revertedWith(
          'MerkleDistributor: Drop already claimed.'
        )
      }
