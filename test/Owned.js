...
describe("Owned contract", function () {
  ...
  beforeEach(async function () {
	...
	it('should revert when owner parameter is passed the zero address', async () => {
		await expect(Owned.deploy(ZERO_ADDRESS)).to.be.revertedWith("Owner address cannot be 0");
	});
	...
	describe('given an instance', () => {
		...
		it('should not nominate new owner when not invoked by current contract owner', async () => {
			...
			await expect(ownedContractInstance.connect(account2).nominateNewOwner(nominatedOwner)).to.be.revertedWith("Only the contract owner may perform this action")
		...
		it('should nominate new owner when invoked by current contract owner', async () => {
			...
			const txn = await ownedContractInstance.connect(account1).nominateNewOwner(nominatedOwner);
		...
		it('should not accept new owner nomination when not invoked by nominated owner', async () => {
			...
			await expect(ownedContractInstance.connect(account2).acceptOwnership()).to.be.revertedWith("You must be nominated before you can accept ownership");
		...
		it('should accept new owner nomination when invoked by nominated owner', async () => {
			const nominatedOwner = account2.address;
			let txn = await ownedContractInstance.connect(account1).nominateNewOwner(nominatedOwner);
			await expect(txn).to.emit(ownedContractInstance, "OwnerNominated");
