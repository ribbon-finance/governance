...
abstract contract ERC20BaseRewardModule is IRewardModule {
    ...
    returns (uint256)
  {
    ...
    return
      ((block.timestamp - funding.updated) * funding.shares) / funding.duration;
  }
  ...
  function _clean(address token) internal {
    ...
    for (uint256 i = 0; i < originalSize; i++) {
      ...
      if (
        ...
        _fundings[token][idx] = _fundings[token][_fundings[token].length - 1];
        _fundings[token].pop();
        removed++;
      }
    }
  ...
  function _distribute(
    ...
    IERC20 rewardToken = IERC20(token);
    amount = (rewardToken.balanceOf(address(this)) * shares) / _shares[token];
